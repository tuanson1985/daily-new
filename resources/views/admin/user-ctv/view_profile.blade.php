    ...
    <div class="modal-footer">
        ...
        <div class="btn-dk">
            ...
            @if(Auth::user()->can(['plus-minus-money']) || Auth::user()->hasRole('admin'))
                <a href="/admin/money?mode=1&field=username&value={{$user->username}}&shop_id={{$user->shop_id}}" class="btn btn-success m-btn m-btn--custom">Cộng tiền</a>
                <a href="/admin/money?mode=0&field=username&value={{$user->username}}&shop_id={{$user->shop_id}}" class="btn btn-danger m-btn m-btn--custom">Trừ tiền</a>
            @endif
