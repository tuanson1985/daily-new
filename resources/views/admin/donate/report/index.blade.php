                ...
                <div class="row">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control datatable-input" id="user_ref" value="{{request('user_ref')}}"
                                   placeholder="{{__('Người nhận')}}">
                        </div>
                    </div>
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            {{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('module.donate.status'),old('status',request('status')),array('id'=>'status','class'=>'form-control datatable-input',))}}
                        </div>
                    </div>
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    "drawCallback": function (settings) {
                        ...
                        $(rows).eq(0).before(
                            ...
                            // '<td colspan=""><b>'+number_format(apiJson.recordsFiltered,'.')+' lượt</b></td>' +
                            '<td colspan=""><b>Tổng: '+number_format(apiJson.totalSumary.total_price,'.')+
                            '<br>'+
                            'Thành công: '+number_format(apiJson.totalSumary.total_success,'.')+'</b></td>' +
                            '<td colspan=""><b>'+number_format(apiJson.totalSumary.total_real_received_price,'.')+'</b></td>' +
                            '<td colspan=""></td>' +
                            '<td colspan=""></td>' +
                            '</tr>',
                        );
                    ...
                    columns: [
                        ...
                        {data: 'user', title: '{{__('Người donate')}}'},
                        {
                            data: 'payment_type', title: '{{__('Phương thức thanh toán')}}',
                            render: function (data, type, row) {
                                if(row.payment_type == 0){
                                    return "{{config('module.donate.payment_type.0')}}";
                                }
                                else if(row.payment_type == 1){
                                    return "{{config('module.donate.payment_type.1')}}";
                                }
                            }
                        },
                        {data: 'user_ref.email', title: '{{__('Người nhận')}}'},
                        {
                            data: 'price', title: '{{__('Số tiền')}}',
                            render: function (data, type, row) {
                                var temp=number_format(row.price,'.');
                                return temp
                            }
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                var temp=number_format(row.real_received_price,'.');
                                return temp
                            }
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                ...
                                var arrConfig= {!! json_encode(config('module.donate.status')) !!};

                                var temp="";

                                if (row.status == 1) {
                                    temp+="<p><b class='text-success'>"+ (arrConfig[row.status]??"")+"</b></p>";
                                } else if (row.status == 2) {
                                    temp+="<p><b class='text-warning'>"+ (arrConfig[row.status]??"")+"</b></p>";
                                } else {
