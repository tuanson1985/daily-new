                ...
                <div class="row">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            {{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('module.withdraw.status'),old('status',request('status')),array('id'=>'status','class'=>'form-control datatable-input',))}}
                        </div>
                    </div>
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {data: 'auth', title: '{{__('Tài khoản')}}'},
                        {data: 'amount', title: '{{__('Số tiền rút')}}'},
                        {data: 'fee', title: '{{__('Phí')}}'},
                        {data: 'amount_passed', title: '{{__('Tổng tiền trừ')}}'},
                        {
                            data: 'status', title: '{{__('Trạng thái')}}',
                            render: function (data, type, row) {

                                if (row.status == 1) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2  label-success \">" + "{{config('module.withdraw.status.1')}}" + "</span>";
                                }
                                else if (row.status == 2) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-warning \">" + "{{config('module.withdraw.status.2')}}" + "</span>";
                                }
                                else if(row.status == 0){
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-danger \">" + "{{config('module.withdraw.status.0')}}" + "</span>";
                                }
                                else{
                                    return "";
                                }
