			...
			<div class="row">
				...
				<div class="form-group col-12 col-sm-6 col-lg-3">
					<div class="input-group">
						...
						<input type="text" class="form-control datatable-input" id="title"
							   placeholder="{{__('Tên nhà phát hành')}}">
					</div>
				</div>
				...
				<div class="form-group col-12 col-sm-6 col-lg-3">
					<div class="input-group">
						...
						{{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('module.server.status'),old('status', isset($data) ? $data->status : null),array('id'=>'status','class'=>'form-control datatable-input',))}}
					</div>
				</div>
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {
                            data: 'title', title: '{{__('Tên nhà phát hành')}}',
                            render: function (data, type, row) {
                                if(row.shop_name != null && row.shop_name.length > 0){
                                    if(row.shop_name.includes("http") ||row.shop_name.includes("https") || row.shop_name.includes("www")){
                                                return '<a title="'+row.title+'" target="_blank" href="'+row.shop_name+'">'+row.title+'</a><br/>';
                                            }
                                            else{
                                                return '<a title="'+row.title+'" target="_blank" href="http://'+row.shop_name+'">'+row.title+'</a><br/>';
                                            }
                                }
                                else{
                                    return row.title;
                        ...
                        {data: 'ipaddress', title: 'Địa chỉ'},
                        {data: 'description', title: 'Điện thoại'},
                        {
