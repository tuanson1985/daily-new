...
@section('content')
    ...
    <div class="card card-custom" id="kt_page_sticky_card">
        ...
        <div class="card-body">
            <!--begin: Search Form-->
            <form class="mb-10" action="{{route('admin.store-card-export.index')}}" method="post">
                {{ csrf_field() }}
                <div class="row">
                    {{--ID--}}
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control datatable-input" id="request_id" value="{{request('request_id')}}" placeholder="{{__('Mã nhà cung cấp')}}">
                        </div>
                    </div>
                    {{--username--}}
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            {{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('module.store-card.status'),old('status',request('status')),array('id'=>'status','class'=>'form-control datatable-input',))}}
                        </div>
                    </div>
                        {{--gate_id--}}
                        <div class="form-group col-12 col-sm-6 col-lg-3">
                            <div class="input-group">
                                ...
                                {{Form::select('gate_id',[''=>'-- Tất cả nhà cung cấp --']+config('module.store-telecom.gate_id'),old('gate_id',request('gate_id')),array('id'=>'gate_id','class'=>'form-control datatable-input',))}}
                            </div>
                        </div>
                          {{--shop_id--}}
            ...
            <div class="row m--margin-bottom-20">
                <div class="col-lg-12 m--margin-bottom-10-tablet-and-mobile" style="font-size: 14px ">
                    Tổng số đơn hàng: <b id="total_record">0</b>
                </div>
                <div class="col-lg-12 m--margin-bottom-10-tablet-and-mobile" style="font-size: 14px ">
                ...
                {{-- <div class="col-lg-12 m--margin-bottom-10-tablet-and-mobile" style="font-size: 14px ">
                    Tổng số thẻ: <b id="total_card">0</b>
                </div> --}}
            </div>
            <br>
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    "drawCallback": function (settings) {
                        ...
                        // var sum = $('#kt_datatable').DataTable().column(7).data();
                        // console.log(sum)
                    },
                    ...
                    columns: [
                        ...
                        {data: 'created_at', title: '{{__('Ngày tạo')}}'},
                        {data: 'username', title: '{{__('Tài khoản')}}'},
                        {data: 'telecom', title: '{{__('Nhà mạng')}}'},
                        {data: 'amount', title: '{{__('Mệnh giá')}}'},
                        {data: 'quantity', title: '{{__('Số lượng')}}'},
                        {data: 'ratio', title: '{{__('Chiết khấu')}}'},
                        {data: 'real_received_price', title: '{{__('Tổng thanh toán')}}'},
                        {
                            data: 'gate_id', title: '{{__('Nhà cung cấp')}}',
                            render: function (data, type, row) {

                                if (row.gate_id == 1) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2  \" data-toggle=\"tooltip\"itle data-original-title=\"" + row.description +"\">" + "{{config('module.store-card.gate_id.1')}}" + "</span> <br/> "+row.request_id;
                                }
                                else {
                                    return "Không tìm thấy cổng";
                                }
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                ...
                                if (row.status == 1) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2  label-success \">" + "{{config('module.store-card.status.1')}}" + "</span>";
                                } 
                                else if (row.status == 2) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-warning \">" + "{{config('module.store-card.status.2')}}" + "</span>";
                                } 
                                else if (row.status == 3) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-info \">" + "{{config('module.store-card.status.3')}}" + "</span>";
                                } 
                                else if (row.status == 4) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-dark \">" + "{{config('module.store-card.status.4')}}" + "</span>";
                                } 
                                else if (row.status == 5) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-primary \">" + "{{config('module.store-card.status.5')}}" + "</span>";
                                } 
                                else if(row.status == 0){
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-danger \">" + "{{config('module.store-card.status.0')}}" + "</span>";
                                }
                                else{
                                    return "";
                                }
