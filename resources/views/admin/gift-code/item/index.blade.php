            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {data: 'code', title: 'Mã code'},
                        {data: 'type', title: 'Loại'},
                        {data: 'started_at', title: '{{__('Ngày bắt đầu')}}'},
                        {data: 'ended_at', title: '{{__('Ngày kết thúc')}}'},
                        {data: 'max_uses', title: '{{__('Tổng lượt nhận')}}'},
                        {data: 'voucher_user_count', title: '{{__('Đã nhận')}}'},
                        {data: 'rest_user', title: '{{__('Còn lại')}}'},
                        {
                            data: 'effect', title: '{{__('Hiệu lực')}}',
                            render: function (data, type, row) {

                                if (row.status_effect == 1) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2  label-success \">" + row.effect + "</span>";
                                } else if (row.status_effect == 2) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-warning \">" + row.effect + "</span>";
                                } else if(row.status_effect == 0){
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-danger \">" + row.effect + "</span>";
                                }
