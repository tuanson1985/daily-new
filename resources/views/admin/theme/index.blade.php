			...
			<div class="row">
				...
				<div class="form-group col-12 col-sm-6 col-lg-3">
					<div class="input-group">
						...
						{{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('module.theme.status'),old('status', isset($data) ? $data->status : null),array('id'=>'status','class'=>'form-control datatable-input',))}}
					</div>
				</div>
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {
                            data: 'content', title: '{{__('Link demo')}}',
                            render: function (data, type, row) {
                                if(row.content == null || row.content.length < 1)
                                {
                                    return "Chưa cập nhật link";
                                }
                                else{
                                    const myArray = row.content.split(",");
