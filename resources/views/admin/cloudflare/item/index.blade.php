...
<!-- delete item Modal -->
<div class="modal fade" id="exPostModal">
    <div class="modal-dialog">
        <div class="modal-content">
            {{Form::open(array('route'=>array('admin.'.$module.'.exposs-excel',0),'class'=>'form-horizontal','method'=>'POST'))}}
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel"> {{__('Xác nhận thao tác')}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i aria-hidden="true" class="ki ki-close"></i>
            ...
            <div class="modal-body">
                {{__('Bạn thực sự muốn xuất excel?')}}
            </div>
            <div class="modal-footer">
                <input type="hidden" name="id" class="id" value=""/>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('Hủy')}}</button>
                <button type="submit" class="btn btn-success">{{__('Xuất excel')}}</button>
            </div>
            {{ Form::close() }}
        </div>
    </div>
...
@section('scripts')
    ...
    <script>
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {data: 'id', title: 'ID'},
                        {
                            data: 'title', title: '{{__('Tên')}}',
                            render: function (data, type, row) {
                                 return row.title;
                            }
                        },
                        {
                            data: 'params', title: '{{__('Thông tin')}}',
                            render: function (data, type, row) {
                                 return row.params;
                            }
                        },
                        {
                            data: 'id', title: '{{__('Xuất excel')}}',
                            render: function (data, type, row) {
                                var html = '';

                                html += '<span class="badge badge-primary exposs_excel" style="cursor: pointer;" data-id='+ row.id +'>Xuất excel</span>';
                                return html;
                            }
                        },
        ...
        jQuery(document).ready(function () {
            ...
            $('body').on('click', '.exposs_excel', function(e) {
                e.preventDefault();
                var id = $(this).data('id');

                $('#exPostModal .id').val(id);
                $('#exPostModal').modal('show');
            })
