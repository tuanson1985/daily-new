...
@section('action_area')
    <div class="d-flex align-items-center text-right">
        <a href="{{route('admin.language-nation.index')}}"
...
@section('content')
    ...
    @if(isset($data))
        {{Form::open(array('route'=>array('admin.language-nation.update',$data->id),'method'=>'PUT','id'=>'formMain','enctype'=>"multipart/form-data" , 'files' => true))}}
    @else
        {{Form::open(array('route'=>array('admin.language-nation.store'),'method'=>'POST','id'=>'formMain','enctype'=>"multipart/form-data"))}}
    @endif
    <div class="row">
        <div class="col-lg-9">
            <div class="card card-custom gutter-b">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        <div class="col-12 col-md-12">
                            <label for="locale">{{ __('Từ khóa') }}:</label>
                            <input type="text" name="locale"
                                   value="{{ old('locale', isset($data) ? $data->locale : null) }}"
                                   placeholder="{{ __('Từ khóa') }}"
                                   class="form-control {{ $errors->has('locale') ? ' is-invalid' : '' }}">
                            @if ($errors->has('locale'))
                                <span class="form-text text-danger">{{ $errors->first('locale') }}</span>
                            @endif
                        </div>
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        <div class="col-12 col-md-12">
                            ...
                            {{Form::select('status',config('module.language-nation.status'),old('status', isset($data) ? $data->status : null),array('class'=>'form-control'))}}
                    ...
                    <div class="form-group row">
                        <div class="col-12 col-md-12">
                            <label for="is_default" class="form-control-label">{{ __('Mặc định') }}</label>
                            {{Form::select('is_default',config('module.language-nation.is_default'),old('is_default', isset($data) ? $data->is_default : null),array('class'=>'form-control'))}}
                            @if($errors->has('is_default'))
                                <div class="form-control-feedback">{{ $errors->first('is_default') }}</div>
                            @endif
                        </div>
