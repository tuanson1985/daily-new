...
@section('action_area')
    <div class="d-flex align-items-center text-right">
        ...
        <div class="btn-group">
            <a href="{{route('admin.language-key.create')}}" type="button"  class="btn btn-success font-weight-bolder">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            {{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('module.language-key.status'),old('status', isset($data) ? $data->status : null),array('id'=>'status','class'=>'form-control datatable-input',))}}
                        </div>
                    </div>
    ...
    <div class="modal fade" id="deleteModal">
        <div class="modal-dialog">
            <div class="modal-content">
                {{Form::open(array('route'=>array('admin.language-key.destroy',0),'class'=>'form-horizontal','id'=>'form-delete','method'=>'DELETE'))}}
...
{{-- Styles Section --}}
@section('styles')
    <link rel="stylesheet" href="/assets/backend/themes/plugins/custom/jexcel/jquery.jexcel.css" type="text/css">
@endsection
{{-- Scripts Section --}}
@section('scripts')
                        ...
                        { data: 'status', title: '{{__('Trạng thái')}}',
                            render: function( data, type, row ) {
                                return "<span class=\"switch switch-outline switch-icon switch-success\">" +
                                             "<label><input type=\"checkbox\" "+(row.status==1?"checked":"") +" name=\"select\" class=\"update_field\" data-action=\"{{route('admin.language-key.update_field')}}\" data-field=\"status\"  data-id=\""+row.id+"\" data-value=\""+row.status+"\"  /><span></span></label>" +
                                        "</span>";

                                {{--if(row.status==1){--}}
                                {{--    return  "<span class=\"label label-pill label-inline mr-2 label-success \">"+"{{config('module.language-key.status.1')}}"+"</span>";--}}
                                {{--}--}}
                                {{--else{--}}
                                {{--    return "<span class=\"label label-pill label-inline mr-2 label-danger \">"+"{{config('module.language-key.status.0')}}"+"</span>";--}}
                                {{--}--}}
                            }
                        },
    ...
    <script src="/assets/backend/themes/plugins/custom/jexcel/jquery.jexcel.js"></script>
