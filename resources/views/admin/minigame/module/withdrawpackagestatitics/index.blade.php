                            ...
                            <div class="dataTables_scrollBody" style="position: relative; overflow: auto; width: 100%;">
                                <table class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline" id="table_main" role="grid" aria-describedby="table_main_info" style="width: 988px;">
                                    ...
                                    <tr role="row " class="b-header">
                                        ...
                                        <th style="background-color: #01a6f5; color: #fff">Tên gói</th>
                                        <th style="background-color: #01a6f5; color: #fff">Tổng lượt rút</th>
                                        <th style="background-color: #01a6f5; color: #fff">Tổng vật phẩm rút</th>
                                        <th style="background-color: #01a6f5; color: #fff">Thành tiền</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @php
                                    ...
                                    @foreach($datatable as $itemCat)
                                        ...
                                        <tr id="parrent{{$itemCat->id}}" class="parrentClass" data-id="{{$itemCat->id}}" style=" background-color: #e0ebf9;font-weight: bold" >
                                            ...
                                            @if($gametypeorder!=null)                                       
                                                ...
                                                <td>
                                                    {{number_format($gametypeorder->sum("price_base"))}}
                                                </td>
                                                @php
                                                    $total_withdraw=$total_withdraw+$gametypeorder->count();
                                                    $total_item=$total_item+$gametypeorder->sum("price");
                                                    $total_item_price=$total_item_price+$gametypeorder->sum("price_base");
                                                @endphp
                                            @else
                                                <td>0</td><td>0</td><td>0</td>
                                            @endif
                                        </tr>
                                        @if($itemCat->gametypechildren!=null)
                                            @foreach($itemCat->gametypechildren as $item)
                                            <tr class="children_{{$itemCat->id}}" data-id="{{$itemCat->id}}" style="display: none">                                                
                                                @php
                                                    $packageorder = $item->packageorder;
                                                @endphp 
                                                @if($packageorder!=null)                                       
                                                    @if(request('started_at')!=null)
                                                        @php
                                                            $packageorder = $packageorder->where('created_at', '>=', \Carbon\Carbon::createFromFormat('d/m/Y H:i:s', request('started_at')));
                                                        @endphp
                                                    @endif                                                
                                                    @if(request('ended_at')!=null)
                                                        @php
                                                            $packageorder = $packageorder->where('created_at', '<=', \Carbon\Carbon::createFromFormat('d/m/Y H:i:s', request('ended_at')));
                                                        @endphp
                                                    @endif
                                                    @if(session('shop_id'))
                                                        @php
                                                            $packageorder = $packageorder->where('shop_id',session('shop_id'));
                                                        @endphp
                                                    @endif
                                                    <td>{{$itemCat->title}}</td>
                                                    <td>{{$item->title}}</td>
                                                    <td>{{number_format($packageorder->count())}}</td>
                                                    <td>{{number_format($packageorder->sum("price"))}}</td>
                                                    <td>{{number_format($packageorder->sum("price_base"))}}</td>                                                
                                                @else
                                                    <td>{{$itemCat->title}}</td>
                                                    <td>{{$item->title}}</td>
                                                    <td>0</td><td>0</td><td>0</td>
                                    ...
                                    <tr style="background-color: #abe7ed;font-weight: bold">
                                        ...
                                        <td>
                                            {{number_format($total_item_price)}}
                                        </td>
                                    </tr>
