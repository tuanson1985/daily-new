...
@section('content')
    ...
    <div class="card card-custom" id="kt_page_sticky_card">
        ...
        <div class="card-body">
            <!--begin: Search Form-->
            <form class="mb-10" action="{{route('admin.withdrawlog-export.index')}}" method="post">
                {{ csrf_field() }}
                <div class="row">
                    {{--title--}}
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            <select id="module-type"  name="module-type"
...
{{--                                <option value="2" >Giao dịch rút tự động</option>--}}
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            {{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('module.minigame.withdraw_status'),old('status', isset($data) ? $data->status : null),array('id'=>'status','class'=>'form-control datatable-input',))}}
                        </div>
                    </div>
                ...
                <div class="row mt-5">
                    ...
                    <div class="col-lg-12 m--margin-bottom-10-tablet-and-mobile" style="font-size: 14px ">
                        Giao dịch thành công: <b id="total_record_complete">0</b> - Giao dịch Thất bại: <b id="total_record_delete">0</b> - Giao dịch chờ xử lý: <b id="total_record_pendding">0</b>
                    </div>
                    <div class="col-lg-12 m--margin-bottom-10-tablet-and-mobile" style="font-size: 14px ">
    ...
    <div class="modal fade" id="deleteRecharge">
        <div class="modal-dialog">
            <div class="modal-content">
                {{Form::open(array('route'=>array('admin.withdraw-item-delete-recharge',0),'class'=>'form-horizontal form-submit-ajax','method'=>'POST'))}}
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> {{__('Xác nhận thao tác')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i aria-hidden="true" class="ki ki-close"></i>
                ...
                <div class="modal-body">
{{--                    {{__('Vui lòng kiểm tra đơn hàng này bên Đại lý trước khi gọi lại ncc')}}--}}
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="id" class="id" >
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('Đóng')}}</button>
                    <button type="submit" class="btn btn-danger m-btn m-btn--custom deleteRechargeModal">{{__('Hủy')}}</button>
                </div>
                {{ Form::close() }}
            </div>
        </div>
    ...
    <div class="modal fade" id="rechargeModal">
        <div class="modal-dialog">
            <div class="modal-content">
                {{Form::open(array('route'=>array('admin.withdraw-item-recharge',0),'class'=>'form-horizontal form-submit-ajax','method'=>'POST'))}}
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> {{__('Xác nhận thao tác')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i aria-hidden="true" class="ki ki-close"></i>
                ...
                <div class="modal-footer">
                    <input type="hidden" name="id" class="id" >
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('Đóng')}}</button>
                    <button type="submit" class="btn btn-success m-btn m-btn--custom rechargeModal">{{__('Xác nhận')}}</button>
                </div>
                {{ Form::close() }}
            </div>
        </div>
    ...
    <div class="modal fade" id="rechargeTimeOut">
        <div class="modal-dialog">
            <div class="modal-content">
                {{Form::open(array('route'=>array('admin.withdraw-item-rechargetimeout',0),'class'=>'form-horizontal form-submit-ajax','method'=>'POST'))}}
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> {{__('Xác nhận thao tác')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i aria-hidden="true" class="ki ki-close"></i>
...
@section('scripts')
    ...
    <script>
        ...
        $('body').on('click', '.btn_recharge_timeout',function(){
            var id = $(this).data('id');

            $('#rechargeTimeOut .id').val(id);
            $('#rechargeTimeOut').modal('show');

        })

        $('body').on('click', '.btn_recharge',function(){
            var id = $(this).data('id');

            console.log(11111111)
            $('#rechargeModal .id').val(id);
            $('#rechargeModal').modal('show');

        })

        $('body').on('click', '.btn_delete_recharge',function(){
            var id = $(this).data('id');

            $('#deleteRecharge .id').val(id);
            $('#deleteRecharge').modal('show');
        ...
        var KTDatatablesDataSourceAjaxServer = function () {
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    buttons: [
{{--                        @if(auth()->user()->can('withdraw-item-recharge'))--}}
{{--                        {--}}
{{--                            text: '<i class="m-nav__link-icon la la-sync-alt"></i> Nạp lại ',--}}
{{--                            action : function(e) {--}}
{{--                                e.preventDefault();--}}
{{--                                var allSelected = '';--}}
...
{{--                        @endif--}}
                        ...
                        {data: 'updated_at', title: '{{__('Thời gian hoàn thành')}}'},
                        {
                            ...
                            render: function (data, type, row) {
                                if (row.payment_type == 13 || row.payment_type == 11 || row.payment_type == 12 || row.payment_type == 14){
                                    if (row.status == 0 || row.status == 3 || row.status == 5 || row.status == 6 || row.status == 77 || row.status == 88) {
                                    ...
                                    } else if(row.status == 1 || row.status == 9) {
                                        return "<span class=\"label label-pill label-inline label-center mr-2 label-warning not-send-text not-send-text"+row.id+"\">" + "Chờ xử lý" + "</span>";
                                    ...
                                    if (row.status == 0) {
                                        ...
                                        return "<span class=\"label label-pill label-inline label-center mr-2 label-warning not-send-text not-send-text"+row.id+"\">" + "{{config('module.minigame.withdraw_status.7')}}" + "</span>";
                                    }
                                    else if(row.status == 9) {
                                        return "<span class=\"label label-pill label-inline label-center mr-2 label-warning not-send-text not-send-text"+row.id+"\">" + "{{config('module.minigame.withdraw_status.9')}}" + "</span>";
                                    }
                                    else {
                        ...
                        { data: 'status',title:'Chuyển trạng thái',
                            render: function ( data, type, row ) {
                                if (row.payment_type == 13 || row.payment_type == 11 || row.payment_type == 12 || row.payment_type == 14){
                                    ...
                                    if(row.status==0){
                                        if (row.payment_type == 1 || row.payment_type == 2 || row.payment_type == 3 || row.payment_type == 8){
                                    ...
                                    @if(auth()->user()->can('withdraw-item-recharge'))
                                    if(row.status==9){
                                        var html_kt = `<div class="d-flex justify-content-center">
                                            <div class="card-toolbar m-0">
                                            <a href="javascript:void(0)" class="btn btn-icon btn-circle btn-sm btn-light-success btn_recharge_timeout" data-id="${row.id}" data-toggle="popover" data-placement="bottom" data-content="Kiểm tra lại đơn hàng">
                                    ...
                                    if (row.status == 7){
                                        var html_kt = `<div class="d-flex justify-content-center">
                                            <div class="card-toolbar mr-2 my-0">
                                            <a href="javascript:void(0)" class="btn btn-icon btn-circle btn-sm btn-light-danger btn_delete_recharge" data-id="${row.id}" data-toggle="modal"><i class="fas fa-times"></i></a>
                                            </div>
                                            <div class="card-toolbar m-0">
                                            <a href="javascript:void(0)" class="btn btn-icon btn-circle btn-sm btn-light-success btn_recharge" data-id="${row.id}" data-toggle="popover" data-placement="bottom" data-content="Kiểm tra lại đơn hàng">
                                                <i class="ki ki-reload icon-nm"></i>
                                            </a>
                                            </div>
                                        </div>`;
                    ...
                    "drawCallback": function (settings) {
                        ...
                        $('#total_record_pendding').text(number_format(apiJson.totalSumary.total_record_pendding,'.'));
