...
@section('action_area')
    @if( Auth::user()->can('withdraw-package-config-create'))
    ...
    <div class="modal fade" id="createModal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> {{__('Thêm gói rút cho shop')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i aria-hidden="true" class="ki ki-close"></i>
                    </button>
                </div>
                <div class="modal-body c-table" style="padding: 12px">

                    @forelse($gametypes as $gametype)

                        <h3 class="title_minigame_config_modal">{{ $gametype->title }}</h3>
                        @if(isset($gametype->children) && count($gametype->children))
                            @php
                                $dataItem = $gametype->children;
                            @endphp
...
{{-- Styles Section --}}
@section('styles')
    <link href="/assets/backend/assets/css/package.css?v={{time()}}" rel="stylesheet" type="text/css"/>
@endsection
{{-- Scripts Section --}}
@section('scripts')
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {
                            data: 'items', title: '{{__('Tên loại game')}}',
                            render: function (data, type, row) {
                                console.log(row)
                                var html = '';

                                if (row.items && row.items.parrent && row.items.parrent.title){
                                    html = row.items.parrent.title;
                                }
                                return html;
                            }
                        },
