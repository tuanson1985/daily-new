...
@section('content')
        ...
        <div class="card-header">
            ...
            <div class="card-toolbar">
                <a href="{{route('admin.feedback')}}">
                    <div class="btn btn-primary btn-primary--icon">
                            <span>
                                <i class="la la-plus"></i>
                                <span>Tạo ý kiến</span>
                            </span>
                    </div>
                </a>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control datatable-input" id="qtv" placeholder="{{__('Người tạo')}}">
                        </div>
                    </div>
                    {{--status--}}
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            <select name="type" id="type" class="form-control datatable-input">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            {{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('module.feedback.status'),old('status', isset($data) ? $data->status : null),array('id'=>'status','class'=>'form-control datatable-input',))}}
                        </div>
                    </div>
    ...
    <div class="modal fade" id="viewModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> {{__('Nội dung góp ý')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i aria-hidden="true" class="ki ki-close"></i>
                    </button>
                </div>
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {data: 'id', title: 'ID'},
                        { data: 'type',title:'Loại', orderable: false, searchable: false},
                        {
                            data: 'contents', title: '{{__('Nội dung')}}',
                            render: function (data, type, row) {
                                if(row.contents.length < 20){
                                    return row.contents;
                                }
                                else {
                                    return row.contents.substr(0, 20) + "...";
                                }
                            }
                        },
                        {
                            data: 'author_name', title: '{{__('Người tạo')}}',
                            render: function (data, type, row) {
                                return row.author_name;
                            }
                        },
                        {{--{--}}
                        {{--    data: 'seen', title: '{{__('Đã xem')}}',--}}
                        {{--    render: function (data, type, row) {--}}
                        {{--        if (row.seen == 0) {--}}
                        {{--            return "<span><i class='fa fa-eye-slash '></i></span>";--}}
                        {{--        }--}}
                        {{--        else{--}}
                        {{--            return "<span><i class=' fa fa-eye ' style='color:#00a651 '></i></span>";--}}
                        {{--        }--}}
                        {{--    }--}}
                        {{--},--}}
                        {
                            ...
                            render: function (data, type, row) {
                                if (row.status == 1) {
                                    ...
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-warning \">" + "{{config('module.'.$module.'.status.3')}}" + "</span>";
                                }
                                else if (row.status == 4) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-warning \">" + "{{config('module.'.$module.'.status.4')}}" + "</span>";
                                }
                                else if (row.status == 5) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-success \">" + "{{config('module.'.$module.'.status.5')}}" + "</span>";
                                }
                                else {
                ...
                datatable.on("click", ".view_toggle", function () {
                    $.ajax({
                        type: "POST",
                        url: '/admin/feedback/getInfoFeedBack',
                        data: {
                            '_token':'{{csrf_token()}}',
                            'id':$(this).attr('rel')
                        },
                        ...
                        success: function (data) {
                            ...
                            if (data.status == "SUCCESS") {
                                $("#contentFeedBack").html(data.data);
                            } else {
                                $("#contentFeedBack").html("Không lấy được nội dung ý kiến");
                            }
                        },
                        error: function (data) {
                            $("#contentFeedBack").html("Không lấy được nội dung ý kiến");
