...
@section('content')
                ...
                <div class="row">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control datatable-input" id="domain"
                                   placeholder="{{__('Shop')}}">
                        </div>
                    </div>
    ...
    <div class="modal fade" id="listItemModal">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Danh sách shop trong nhóm</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i aria-hidden="true" class="ki ki-close"></i>
                    </button>
                </div>
...
@section('scripts')
    ...
    <script>
                    ...
                    buttons: [

                        {
                            ...
                            action : function(e) {
                                ...
                                if(total<=0){
                                    toast("Vui lòng chọn dòng để thực hiện thao tác", 'error');
                                    return;
                                }
        ...
        var KTDatatablesDataSourceAjaxServer1 = function (id) {
            var initTable2 = function (id) {
                datatable = $('#showItem_datatable').DataTable({
                    ...
                    "order": [[1, "desc"]],
                    ajax: {
                        url: '{{route("admin.shop-group.get-shop-in-group")}}' + '?shop_group='+id+'&ajax=1',
                        type: 'GET',
                        data: function (d) {
                            d.id = $('#id').val();
                        }
        ...
        jQuery(document).ready(function () {
            ...
            function UpdateStatusClient(id){
                $.ajax({
                        type: "POST",
                        url: "{{route('admin.shop-group.update-stt')}}",
                        data: {
                            '_token':'{{csrf_token()}}',
                            'id':id,
                        },
            ...
            $('body').on('click','.btn-show-item',function(e){
                e.preventDefault();
                var id = $(this).data('id');
                $('#listItemModal .nav-search-in-value').css('display','none');
                $('#listItemModal #result-search .rs-item').remove();
                $('#listItemModal #result-search hr').remove();
                $('#txtSearch').val('');
                showItem(id);
            });
            function showItem(id){
                $('#listItemModal .modal-body #id-group').val(id);
                KTDatatablesDataSourceAjaxServer1.init(id);
