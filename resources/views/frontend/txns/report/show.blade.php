...
<div class="modal-header">
    <h5 class="modal-title" id="exampleModalLabel"> {{ __('Giao dịch') }} #{{$datatable->id}}</h5>
...
<div class="modal-body">
    @php
        $user = str_replace('tt_', '', $datatable->user->username);
    @endphp
    <table class="table table-striped">
        <tbody>
        <tr>
            <th colspan="2"><b>{{ __('Thông tin giao dịch') }} #{{$datatable->id}}</b></th>
        </tr>
        <tr>
            <td width="40%">{{ __('Thời gian:') }}</td>
            <td>{{\App\Library\Helpers::FormatDateTime('d/m/Y H:i:s',$datatable->created_at)}}</td>
        </tr>
        <tr>
            <td>{{ __('Tài khoản:') }}</td>
            <td>
                <a href="#" class="load-modal" rel="#">
                    {{$user}}
                </a>
            </td>
        </tr>
        ...
        @if($datatable->processor_username!="")
            <tr>
                <td>{{ __('Người thực hiện:') }}</td>
                <td>
                    <a href="#" class="load-modal" rel="#">
                        @php
                            $processor_username = str_replace('tt_', '', $datatable->processor_username);
                        @endphp
                        {{$processor_username}}
                    </a>
                </td>
    ...
    @elseif($datatable->trade_type=="charge")
                <table class="table table-striped">
                    <tbody>
                    <tr>
                        <th colspan="2"><b>{{ __('Thông tin nạp thẻ') }} #{{$datatable->txnsable->id??""}}</b></th>
                    </tr>

                    <tr>
                        <td width="40%">{{ __('Tài khoản:') }}</td>

                        <td>
                            <a href="#" class="load-modal" rel="#">{{$user}}</a>
                        </td>
                    </tr>
                    <tr>
                        <td>{{ __('Cổng gạch thẻ:') }}</td>
