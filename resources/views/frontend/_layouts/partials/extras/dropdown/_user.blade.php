    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap p-8 bgi-size-cover bgi-no-repeat rounded-top" style="background-image: url('{{ asset('assets/backend/themes/media/misc/bg-1.jpg') }}')">
        <div class="d-flex align-items-center mr-2">
            ...
            <div class="symbol bg-white-o-15 mr-3">
                <span class="symbol-label text-success font-weight-bold font-size-h4">{{strtoupper(substr(auth('frontend')->user()->username, 0, 1))}}</span>
            </div>

            {{-- Text --}}
            @include('frontend._layouts.partials.extras._control_money')
        </div>
    </div>
@endif
...
<div class="navi navi-spacer-x-0 pt-5">
    {{-- Item --}}
    <a href="{{route('frontend.profile')}}" class="navi-item px-8">
        <div class="navi-link">
    ...
    {{-- Item --}}
    <a href="{{route('frontend.profile',['#kt_user_edit_tab_3'])}}" class="navi-item px-8">
        <div class="navi-link">
            <div class="navi-icon mr-2">
    ...
    <div class="navi-footer  px-8 py-5">
        <a href="#" target="_blank" class="btn btn-light-primary font-weight-bold"  onclick="event.preventDefault();
                                                     document.getElementById('logout-form-frontend').submit();">{{__('Đăng xuất')}}</a>
