...
<head>
    ...
    {{-- Title Section --}}
    <title>{{ config('app.name') }} | @yield('title', $page_breadcrumbs[0]['title'] ?__($page_breadcrumbs[0]['title']): '')</title>
    ...
    {{-- Favicon --}}
    <link rel="shortcut icon" href="{{ asset('assets/frontend/themes/media/logos/favicon.ico') }}" />
    ...
    @endforeach
    <link href="/assets/frontend/assets/css/fixcss.css" rel="stylesheet" type="text/css"/>
    {{-- Includable CSS --}}
    @yield('styles')
</head>
...
@if (config('layout.page-loader.type') != '')
    @include('frontend._layouts.partials._page-loader')
@endif

@include('frontend._layouts.base._layout')
...
<script src="{{ mix('assets/frontend/assets/vuejs/admin.js') }}?v=1.5"></script>
@endif
{{-- Global Theme JS Bundle (used by all pages)  --}}
@foreach(config('layout.resources.js') as $script)
    <script src="{{ asset($script) }}?v={{time()}}" type="text/javascript"></script>
@endforeach

{{-- Includable JS --}}
@yield('scripts')
<script src="/assets/frontend/assets/js/custom.js?v=1.0"></script>
...
@include('frontend._layouts.includes.notifications')
@include('frontend._layouts.includes.feedbackcount')
