...
use App\Jobs\ServiceAuto\DefragmentSanJob;
...
class DefragmentSan extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'defragmentsan:cron';
     ...
     * @var string
     */
    protected $description = 'dồn nick san';
    ...
    public function handle()
    {
        ...
        $txt = Carbon::now().":chạy cronjob: log-job-defragment san chạy job defragment thành công";
        \File::append($path.Carbon::now()->format('Y-m-d').".txt",$txt."\n");

        $bot = Roblox_Bot_San::query()
            ->where('status',6)
        ...
        if (!isset($bot)){
            ...
            $txt = Carbon::now().":chạy cronjob:[SAN] log-job-defragment error không tìm thấy bot cần xử lý";
            \File::append($path.Carbon::now()->format('Y-m-d').".txt",$txt."\n");
        ...
        if ($coin < 1){
            ...
            $message .= "\n";
            $message .= "<b>[SAN] Dồn robux tài khoản : ".$account." thành công</b>";
            $message .= "\n";
            ...
            if (isset($result)){
                if (isset($result->status)){
                    ...
                    if($result->status==1){
                        ...
                        $message = "Thời gian: <b>".Carbon::now()->format('d-m-Y H:i:s')."</b>";
                        $message .= "\n";
                        $message .= "<b>[SÀN] Dồn robux tài khoản : ".$account." thành công</b>";
                        $message .= "\n";
                        $message .= "<b>Id đơn hàng : ".$id_pengiriman." </b>";
                        $message .= "\n";
                    ...
                    elseif($result->status==2){
                        ...
                        $message = "Thời gian: <b>".Carbon::now()->format('d-m-Y H:i:s')."</b>";
                        $message .= "\n";
                        $message .= "<b>[SAN] Dồn robux tài khoản : ".$bot->acc." thất bại</b>";
                        $message .= "\n";
                    ...
                    elseif($result->status==999){
                        ...
                        $message = "Thời gian: <b>".Carbon::now()->format('d-m-Y H:i:s')."</b>";
                        $message .= "\n";
                        $message .= "<b>[SAN] Dồn robux tài khoản : ".$bot->acc." thất bại</b>".$result->message??"";
                        $message .= "\n";
