...
class DefragmentSanJob implements ShouldQueue
{
    ...
    public function handle() {
        ...
        try {
            ...
            if ((int)$bot->coin >= 10){
                $coin = round((int)$bot->coin / 10) * 10;
            }else{
                $coin = (int)$bot->coin??0;
            }

            $result = RobloxGate::ProcessBuyGamePassNew($uname,$coin,$bot->cookies,$order_id);
            DB::beginTransaction();
            try {
                    ...
                    if (isset($result->status)){
                        ...
                        if($result->status==1){
                            ...
                            if ($bot->coin <= 0){
                                ...
                                $message .= "\n";
                                $message .= "<b>[SAN] Dồn robux tài khoản : ".$bot->acc." thành công</b>";
                                $message .= "\n";
