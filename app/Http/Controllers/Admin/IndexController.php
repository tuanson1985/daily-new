...
use App\Exports\ATMEToltalxport;
use App\Exports\ATMExport;
use App\Exports\BanTheExport;
use App\Exports\MinigameExport;
use App\Exports\NapTheExport;
use App\Exports\NapTheTotalExport;
use App\Exports\ServiceAutoExport;
...
class IndexController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.index-v2', compact('page_title', 'page_breadcrumbs', 'category_access'));
    }
...
//                                    ->selectRaw('AVG(IF(status = 1 ,price,0)) as price_avg')
    ...
    public function dashboardExportExcel(Request $request){

        if (!$request->filled('module')){
            return redirect()->back()->withErrors(__('Không tìm thấy module để xuất excel'));
        }

        $module = $request->get('module');

        if (!$request->filled('started_at')){
            return redirect()->back()->withErrors(__('Vui lòng chọn thời gian bắt đầu'));
        }
        $started_at = $request->get('started_at');
        if (!$request->filled('ended_at')){
            return redirect()->back()->withErrors(__('Vui lòng chọn thời gian kết thúc'));
        }
        $ended_at = $request->get('ended_at');

        $dateStarted = Carbon::createFromFormat('d/m/Y H:i:s', $started_at);
        $formatStartedDate = $dateStarted->format('d-m-Y');

        $dateEnd = Carbon::createFromFormat('d/m/Y H:i:s', $ended_at);
        $formatEndDate = $dateEnd->format('d-m-Y');
        if (!$request->filled('type')){
            return redirect()->back()->withErrors(__('Vui lòng chọn loại xuất dữ liệu'));
        }
        ...
        if ($module == "service"){
            ...
            $export = new ServiceExport($started_at,$ended_at,$status);
            return \Excel::download($export, 'dich vu thu cong ' . $formatStartedDate . ' _ '. $formatEndDate. '.xlsx');
        }elseif ($module == "service-auto"){
            if(!Auth::user()->can('dashboard-export-excel-service-auto')){
                return redirect()->back()->withErrors(__('Không có quyền truy cập'));
            }
            $export = new ServiceAutoExport($started_at,$ended_at,$status);
            return \Excel::download($export, 'dich vu tu dong ' . $formatStartedDate . ' _ '. $formatEndDate. '.xlsx');
        }
    }

    public function exportDashboard(Request $request){
        $page_title = 'Dashboard export';
        $page_breadcrumbs = [
            ['page' => '1',
                'title' => 'Home',
            ],
        ...
        ActivityLog::add($request, 'Truy cập dashboard index export');
        return view('admin.index-export', compact('page_title', 'page_breadcrumbs'));
    }
