...
class UserCTVController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->page_breadcrumbs[] = [
            'page' => route('admin.user-ctv.index'),
    ...
    public function index(Request $request)
    {
        ...
        ActivityLog::add($request, 'Truy cập danh sách user-ctv');
        if($request->ajax) {

            if ($request->filled('pemission_service_speed') && $request->get('pemission_service_speed') == 1){
// thông tin dịch vụ

                $datatable = Item::query()->with(array('groups' => function ($query) {
                    $query->where('module', 'service-category');
                ...
                return \datatables()->eloquent($datatable)
                    ...
                    ->editColumn('ratio', function ($data) {
                        $html ='';
                        if (isset($data->idkey) && $data->idkey != ''){
                            $html .= "<input type='text' class='form-control ratio_input' />";
                        }
                        return $html;
                    })
            ...
            return \datatables()->eloquent($datatable)->whitelist(['id'])
                ...
                ->addColumn('action', function($row) {
                    ...
                    if(auth()->user()->hasRole('admin') || auth()->user()->can('user-qtv-edit')){
                        $temp.= "<a href=\"".route('admin.user-ctv.edit',$row->id)."\"  rel=\"$row->id\" class=\"btn btn-sm  btn-icon btn-hover-text-white btn-hover-bg-primary \" title=\"Sửa\"><i class=\"la la-edit\"></i></a>";
                    };
                    if ($row->status == 0) {
                    ...
                    if(config('module.user-qtv.need_set_permission')){
                        if(auth()->user()->can('attach-permission-qtv')){
                            $temp.= "<a href=\"".route('admin.user-ctv.set_permission',$row->id)."\"  rel=\"$row->id\" class=\"btn btn-sm  btn-icon btn-hover-text-white btn-hover-text-white btn-hover-bg-danger \" title=\"Phân quyền\"><i class=\"la la-sitemap\"></i></a>";
                        }
                    }
        ...
        return view('admin.user-ctv.index')->with('page_breadcrumbs', $this->page_breadcrumbs)
    ...
    public function create(Request $request)
    {
        ...
        ActivityLog::add($request, 'Vào form create user-ctv');
        return view('admin.user-ctv.create_edit')
...
//        $input['account_type'] = 2;
        ...
        ActivityLog::add($request, 'Tạo mới thành công user-ctv #'.$data->id);

        if ($request->filled('submit-close')) {
            return redirect()->route('admin.user-ctv.index')->with('success', __('Thêm mới thành công !'));
        } else {
    ...
    public function edit(Request $request,$id)
    {
        ...
        ActivityLog::add($request, 'Vào form edit user-ctv #'.$data->id);
        return view('admin.user-ctv.create_edit')
            ->with('data', $data)
            ->with('user', $user)
            ->with('page_breadcrumbs', $this->page_breadcrumbs)
...
//        $input['account_type'] = 2;
        ...
        ActivityLog::add($request, 'Cập nhật thành công user-ctv #'.$data->id);
        if($request->filled('submit-close')){
            return redirect()->route('admin.user-ctv.index')->with('success',__('Cập nhật thành công !'));
        }
        else {
    ...
    public function view_profile(Request $request){
        ...
        if($request->shop_id){
            $data=User::where('username',$username)->where("account_type",3)->where('shop_id',$request->shop_id)->where('status',1)->firstOrFail();
        }
        else{
...
//        }
    ...
    public function set_permission(Request $request,$id){
        ...
        ActivityLog::add($request, 'Vào form cập nhật permission user-ctv #'.$data->id);
        return view('admin.user-ctv.set_permission')
            ->with('page_breadcrumbs', $this->page_breadcrumbs)
            ->with('data', $data)
            ->with('user', $user)
            ->with('shops', $shops)
    ...
    public function AccessUser(Request $request){
        ...
        $id = $request->id;
        $description = $request->description;
        $user = User::where('account_type',3)->where('id',$id)->firstOrFail();
        ...
        $url = 'https://'.$shop->domain.'/user/access?sign='.$sign_access;
        // lấy thông tin IP và user_angent người dùng
        $ip = $request->getClientIp();
        $user_agent = $request->userAgent();
        $message = "Thời gian: <b>".Carbon::now()->format('d-m-Y H:i:s')."</b>";
        $message .= "\n";
        $message .= "Tài khoản qtv <b>".Auth::user()->username."</b> vừa truy cập thành công tài khoản thành viên: <b>".$user->username."</b> - ".$user->shop->domain;
        $message .= "\n";
        $message .= "Lý do: <b>".$description."</b>";
        $message .= "\n";
        $message .= "IP: <b>".$ip."</b>";
        $message .= "\n";
        $message .= "User_agent: <b>".$user_agent."</b>";
        Helpers::TelegramNotify($message,config('telegram.bots.mybot.channel_noty_access_user'));
    ...
    public function exportExcel(Request $request){
        ...
        $export = new CTVExport($request);
        return \Excel::download($export, 'Thống kê ctv_ ' . time() . '.xlsx');
    }

    public function post_set_permission_speed(Request $request){
        ...
        $validator = Validator::make($request->all(), [
            ...
            'ids.required' => __('Vui lòng chọn dịch vụ'),
            'ratios.required' => __('Vui lòng chọn tỷ giá'),
            'user_ids.required' => __('Vui lòng chọn user'),
        ]);
        ...
        if(!$auth_user->hasRole('admin')){
            return response()->json([
                'message' => 'Tài khoản admin mới được sử dụng tính năng này',
                'status' => 0
            ],200);
        }
        ...
        $ratios = explode(',',$request->get('ratios'));
        $display_info_role = $request->get('display_info_role')??'';
        $view_role = $request->get('view_role')??'';
        $accept_role = $request->get('accept_role')??'';

        $ratio_all = $request->get('ratio_all')??'';

        if (!$ratio_all){
            if (count($ids) != count($ratios)){
                return response()->json([
                    'message' => 'Lỗi cấu hình vui lòng thử lại',
                    'status' => 0
                ],200);
            }
        }

        if($display_info_role == 0 && $view_role == 0 && $accept_role == 0){
            return response()->json([
        ...
        $users = User::query()->where('status',1)->where("account_type",3)->whereIn('id',$user_ids)->get();

        foreach ($users??[] as $user){
            //phân quyền cho dịch vụ
            if ($display_info_role == 1){
                $permission_service['display_info_role'] = $ids;
            }
            if ($view_role == 1){
                $permission_service['view_role'] = $ids;
            }
            if ($accept_role == 1){
                $permission_service['accept_role'] = $ids;
                foreach ($ids??[] as $key => $id){
                    ...
                    $permission_service['ratio_' . $id] = $ratio;
                }
            }

            $service_access= ServiceAccess::where('user_id',$user->id)->where('module','user')->first();
            ...
            if($service_access){
                ...
                if (!empty($service_access->params)){
                    ...
                    if (!empty($params->accept_role)){
                        $service_accept_roles = $params->accept_role;
                        // Gộp hai mảng và lọc các giá trị trùng lặp
                        if ($accept_role == 1){
                            $merged_array_accept_role = array_values(array_unique(array_merge($service_accept_roles, $permission_service['accept_role'])));
                            $permission_service['accept_role'] = $merged_array_accept_role;

                            foreach ($service_accept_roles??[] as $service_accept_role){
                                if (!in_array($service_accept_role,$ids)){
                                    $attribute = 'ratio_' . $service_accept_role;
                                    $permission_service['ratio_' . $service_accept_role] = $params->$attribute;
                                }
                            }
                        }else{
                            foreach ($service_accept_roles??[] as $service_accept_role){
                    ...
                    if (!empty($params->view_role)){
                        $service_view_role = $params->view_role;
                        // Gộp hai mảng và lọc các giá trị trùng lặp
                        if ($view_role == 1){
                            $merged_array_view_role = array_values(array_unique(array_merge($service_view_role, $permission_service['view_role'])));
                            $permission_service['view_role'] = $merged_array_view_role;
                            // In kết quả ra màn hình
                        }else{
                            $permission_service['view_role'] = $service_view_role;
                        }

                    }
                    if (!empty($params->display_info_role)){
                        $service_display_info_role = $params->display_info_role;
                        // Gộp hai mảng và lọc các giá trị trùng lặp
                        if ($display_info_role == 1){
                            $merged_array_display_info_role = array_values(array_unique(array_merge($service_display_info_role, $permission_service['display_info_role'])));
                            $permission_service['display_info_role'] = $merged_array_display_info_role;
                            // In kết quả ra màn hình
                        }else{
                            $permission_service['display_info_role'] = $service_display_info_role;
                        }
                    }
