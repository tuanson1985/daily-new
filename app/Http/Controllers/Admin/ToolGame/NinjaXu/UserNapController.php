...
class UserNapController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if( $this->module!=""){
            $this->page_breadcrumbs[] = [
                'page' => route('admin.ninjaxu-usernap.index'),
                'title' => __('Bán xu ninja shool - Thông tin bot nạp')
            ];
        }
    }
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return $datatable = \datatables()->eloquent($model)
                ...
                ->addColumn('action', function ($row) {
                    $temp = "<a href=\"" . route('admin.ninjaxu-usernap.edit', $row->id) . "\"  rel=\"$row->id\" class=\"m-portlet__nav-link btn m-btn m-btn--hover-info m-btn--icon m-btn--icon-only m-btn--pill \" title=\"Sửa\"><i class=\"la la-edit\"></i></a>";
        ...
        return view('admin.toolgame.ninjaxu.usernap.index')
    ...
    public function create()
    {
        ...
        return view('admin.toolgame.ninjaxu.usernap.create_edit')
    ...
    {
        $input=$request->all();
        $item=NinjaXu_AccNap::create($input);

        if($request->filled('submit-new')){
            $response=redirect()->route('admin.ninjaxu-usernap.create');
        }
        else {
            $response=redirect()->route('admin.ninjaxu-usernap.index');
        }
    ...
    {
        //$datatable = NinjaXu_AccNap::findOrFail($id);
        //return view('admin.toolgame.ninjaxu.usernap.show', compact('datatable'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = NinjaXu_AccNap::findOrFail($id);
        return view('admin.toolgame.ninjaxu.usernap.create_edit', compact('data'))
            ->with('module', $this->module)
            ->with('page_breadcrumbs', $this->page_breadcrumbs);
    ...
    public function update(Request $request,$id)
    {
        ...
        $item = NinjaXu_AccNap::findOrFail($id);
        $input = $request->all();
        $item->update($input);
    ...
    public function destroy(Request $request)
    {
        ...
        NinjaXu_AccNap::whereIn('id',$input)->delete();
