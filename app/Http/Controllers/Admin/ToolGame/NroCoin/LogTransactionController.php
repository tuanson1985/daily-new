...
class LogTransactionController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->module='nrocoin-logtransaction';
        ...
        if( $this->module!=""){
            $this->page_breadcrumbs[] = [
                'page' => route('admin.nrocoin-logtransaction.index'),
                'title' => __('Bán vàng NRO - Thống kê giao dịch')
            ];
        }
            ...
            return $datatable = \datatables()->eloquent($datatable)
                ...
                ->with('totalSumary', function() use ($datatableTotal) {
                    return $datatableTotal=$datatableTotal->first([
                        DB::raw('COUNT(nrocoin_khachhang.id) as total_record'),
                        DB::raw('SUM(IF(nrocoin_khachhang.status = \'danhan\', nrocoin_khachhang.money, 0)) as total_coin'),
                        DB::raw('SUM(IF(nrocoin_khachhang.status = \'danap\', nrocoin_khachhang.money, 0)) as total_coin_nap'),
                        DB::raw('SUM(IF(nrocoin_khachhang.thoivangsau < nrocoin_khachhang.thoivangtruoc, nrocoin_khachhang.thoivangtruoc - nrocoin_khachhang.thoivangsau, 0)) as total_gold'),
                        DB::raw('SUM(IF(nrocoin_khachhang.thoivangsau > nrocoin_khachhang.thoivangtruoc, nrocoin_khachhang.thoivangsau - nrocoin_khachhang.thoivangtruoc, 0)) as total_gold_nap'),
//                        DB::raw('SUM(order.price) as total_price')
        ...
        return view('admin.toolgame.nrocoin.logtransaction.index')
    ...
    public function MakeQuery(Request $request)
    {
        ...
        if ($request->filled('started_at')) {
            $datatable->where('nrocoin_khachhang.updated_at', '>=', Carbon::createFromFormat('d/m/Y H:i:s', $request->get('started_at')));
        }
        if ($request->filled('ended_at')) {
            $datatable->where('nrocoin_khachhang.updated_at', '<=', Carbon::createFromFormat('d/m/Y H:i:s', $request->get('ended_at')));
        }
...
//        }
        ...
        return Excel::download(new ExportData($data,view('admin.toolgame.nrocoin.logtransaction.excel')), 'Thống kê giao dịch tool nrocoin_ ' . time() . '.xlsx');
    }
