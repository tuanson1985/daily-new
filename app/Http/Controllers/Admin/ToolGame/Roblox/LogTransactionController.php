    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $datatable = Roblox_Order::query()
                ->whereIn('type_order',[1,2,3,4,5,6,7])
                ->where('type_order',3)
            ...
            return $datatable = \datatables()->eloquent($datatable)
                ...
                ->editColumn('payment_type', function ($row) {
                    $payment_type = "ĐAI LÝ";
                    if (isset($row->order) && isset($row->order->payment_type) && $row->order->payment_type == 2){
        ...
        return view('admin.toolgame.roblox.logtransaction.index')
