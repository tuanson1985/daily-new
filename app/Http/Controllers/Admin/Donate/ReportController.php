...
class ReportController extends Controller
{
    ...
    public function __construct(Request $request)
    {
        ...
        $this->middleware('permission:donate-report-list');
        ...
        $this->page_breadcrumbs[] = [
            'page' => route('admin.donate-report.index'),
            'title' => __('Thống kê donate')
        ];
    }
    ...
    public function index(Request $request)
    {
            ...
            if ($request->filled('user_ref')) {
                $datatable->whereHas('user_ref', function ($query) use ($request) {
                    $query->where(function ($qChild) use ($request){
                        ...
                        $qChild->orWhere('fullname_display', $request->get('user_ref'));
                    });
            ...
            return \datatables()->eloquent($datatable)
                ...
                ->with('totalSumary', function() use ($datatable) {
                    return $datatable->first([
                        ...
                        DB::raw('SUM(IF(status = 1, price, 0)) as total_success'),
                        DB::raw('SUM(IF(status != 1, price, 0)) as total_wrong_amount'),
        ...
        return view('admin.donate.report.index')
            ->with('module', null)
