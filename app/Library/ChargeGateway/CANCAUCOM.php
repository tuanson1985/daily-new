...
Class CANCAUCOM
{
    public static function API($partner_id,$partner_key,$card_type,$card_code,$card_serial,$amount,$request_id,$domain){
        try{
            $url = config('charge.cancaucom.url');
            ...
            if ($result && isset($result->status)) {
                $path = storage_path() ."/logs/charge_ccc/";
                if(!\File::exists($path)){
                ...
                if ($response_code==2) {
                    ...
                    if($response_code == 996){
                        $text_tele = "API NẠP THẺ CCC: ".$domain.' - '.$message;
    ...
    public static function createUser($username,$password){
        try{
            $url = config('charge.cancaucom.url_create_user');
            $data = array();
            ...
            curl_close($ch);
            $result = json_decode($resultRaw);
            $myfile = fopen(storage_path() ."/logs/log-create_user_ccc-".Carbon::now()->format('Y-m-d').".txt", "a") or die("Unable to open file!");
