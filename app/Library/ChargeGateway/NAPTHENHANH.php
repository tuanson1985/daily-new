...
Class NAPTHENHANH
{
    public static function API($partner_id,$partner_key,$card_type,$card_code,$card_serial,$amount,$request_id,$domain){
        try{
            $url = config('charge.napthenhanh.url');
            ...
            if ($result && isset($result->status)) {
                $path = storage_path() ."/logs/charge_ntn/";
                if(!\File::exists($path)){
                ...
                if ($response_code==2) {
                    ...
                    if($response_code == 996){
                        $text_tele = "API NẠP THẺ NTN: ".$domain.' - '.$message;
    ...
    public static function createUser($username,$password){
        try{
            $url = config('charge.napthenhanh.url_create_user');
            $data = array();
            $data['username'] = $username;
            $data['email'] = null;
            $data['phone'] = null;
            $data['password'] = $password;
            // $data['sign'] = '9xvcwstiquvhyxxumj0s';
            ...
            curl_close($ch);
            $result = json_decode($resultRaw);
            $myfile = fopen(storage_path() ."/logs/log-create_user_ntn-".Carbon::now()->format('Y-m-d').".txt", "a") or die("Unable to open file!");
