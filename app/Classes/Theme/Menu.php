...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        elseif (isset($item['section'])) {
            if(isset($item['permission'])){
                if((Auth::user() && Auth::user()->hasRole('admin')) || (auth('frontend')->user() && auth('frontend')->user()->hasRole('admin'))){
                    ...
                    foreach($permission as $item_p){
                        if((Auth::user() && Auth::user()->can($item_p)) || (auth('frontend')->user() && auth('frontend')->user()->can($item_p))){
                            $i = 1;
                            break;
                        }
                        else{
    ...
    public static function renderHorMenu($item, $parent = null, $rec = 0)
    {
        ...
        if (isset($item['separator'])) {
            ...
            if(isset($item['permission'])){
                if(!Auth::user()->hasRole('admin') || !Auth::user()->can([$item['permission']])){
                    return false;
                }
                else{
    ...
    public static function isActiveVerMenuItem($item, $page, $rec = 0)
    {
        ...
        if(isset($item['route']) && $item['route'] !=""){
            ...
            if(strpos($item['route'],$segment)>-1){
                return true;
            }
        ...
        //$page is current path





        //dd($url,$url."/*",request()->is("admin-1102/*"));
        if ($url == url($page)) {
